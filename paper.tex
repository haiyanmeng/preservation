...
CMSSW contains many different tools, libraries, and utilities.  No single code uses anywhere close to all of these.  But, because it is widely used within the experimental researchers, it is common for users and developers to simply expect that a particular version of the entire repository is available.
...
the CMS detector readout electronics and reconstructed world-wide, which is then processed through various algorithms derive signatures of individual particles.
...
but only 448.3MB in source are checked out, and the software actually used only
...
To do this, the application was repeated with the help of Original Script (as shown in Figure~\ref{fig:version-evolution}) on the original machine. 
Then one reduced package was generated and verified with the help of Parrot on the original machine.
...
Then based on the successful execution of Original-Script, Reduced-Package is evaluated - one package containing necessary dependencies is generated, and then the time consumption and data size is analyzed. 

To measure time consumption, we counted the time used to obtain remote software dependencies, build the environment, and analyze the dataset. We also counted the time used to obtain the filename list and generate the package. To measure data size, we can easily figure out the size of each data and code source inside the package under Reduced-Package.
...
All the machines adopt the x86\_64 hardware platform and Linux OS.
...
Regardless, we demonstrate that the application can be repeated with the help of the reduced package on one different machine with the required hardware architecture and OS distribution version.
...
OCI-1148330 (SI2) and PHY-1312842.  We also thank the support and help of the Center for Research Computing (CRC) of University of Notre Dame.
