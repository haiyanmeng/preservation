...
\begin{figure}
\centering
\epsfig{file=data-process.eps, height=3in, width=3in}
\caption{Data Process}
\label{fig:data-process}
\end{figure}

The data process is shown in Figure~\ref{fig:data-process}.
...
\begin{figure*}
\centering
\epsfig{file=overview-of-architecture.eps, height=6in, width=7in}
...
Figure~\ref{fig:Overview of architecture} illustrates all the dependencies involved in the program, which can be divided into two categories: clear dependencies and implicit dependencies. Clear dependencies include data dependencies and direct software dependencies which can be extracted directly from the analysis program. Implicit dependencies include indirect software dependencies which is necessary for the successful execution of direct software dependencies and other implicit dependencies. For example, we know Git is one direct software dependency from the analysis program, which further depends on perl, python, rsync, openssh-clients and other packages. The analysis program accesses one file File1, which is one soft link file further referring to File2.
...
\begin{figure}
\centering
\epsfig{file=solution3.eps, height=3in, width=3in}
\caption{Relationship of Roles}
\label{fig:solution3}
\end{figure}

The relationship of different roles involved experiment preservation and reproduction is shown in Figure~\ref{fig:solution3}.
The original author is responsible for generating one package based on the successful execution sandbox with the help of the packaging utility. Then the package, together with the map file and description file will be uploaded into the archive. When another scholar wants to repeat the experiment, one copy of the package and its corresponding map file will be downloaded into the new machine.
...
In the map relationship shown above, During the packaging process, we noticed
that it is impossible to copy the files under certain directories like /dev.
...
current allocation of process ID. As for these two file categories, letting
...
the necessary environment variables, svn login, the actual analysis command
...
Parrot traps all the system calls and copies each accessed file (except for common dependencies) into one new package.
...
The same thing happens to network resources.
...
\begin{figure*}
\centering
\epsfig{file=preservation-integration.eps, height=4in, width=6in}
...
The architecture of preservation integration of multiple programs is shown in ~\ref{fig:Preservation integration}. Script 1 and Script 2, sharing one same dependency B, belongs two different programs. Without preservation integration, B will be preserved twice in the archive, which is space-consuming. To improve the utilization efficiency of storage resources, preservation integration of multiple programs is necessary. In the archive, only one copy of B is stored, which is referred by both scripts 1 and script 2.
...
as network resources is more efficient than preserving it into one package.

\subsection{Mining of Dependencies} 

The mining degree of data and software dependencies greatly determines the
repeatability of one experiment. Data dependencies is easy to figured out.
However, the situation of software dependencies is complex. The software
dependencies include OS, hardware platform, software directly forked inside the
progam and software indirectly forked by directly forked software. The first
...
to preserve the file needs to be considered carefully. If the size of the file
is 1KB or 1MB, preserving the content is acceptable. If the size of one file is
1GB, preserving the whole file just for the access of its metadata is
inefficient.  
...
and unpredictable. Lessons: all the items under one directory must be copied
into the package to ensure the semantics of the original program. However, the
...
directory and record the origial metadata into the database. 
...
authority need to be modified. However, with the generated package and map file, we can repeat the experiment directly on the same machine.
%here is the new content-hmeng
