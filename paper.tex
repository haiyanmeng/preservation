...
HDFS filesystem with 11.6TB of data, but only 43.3GB of that are the input Ntuples,
and of those only 20GB are actually consumed by the program. The CMSSW repository is 88.1GB in total
...
as \verb$GIT$ and \verb!$CMSSW!, while the map file indicates where they
...
trusted remote repositories.  Whether this is advisable is a judgement that
...
he modification of the package only introduces the minimal changes of the map file on the client side.
...
exisiting programs to a variety of remote I/O systems, such as HTTP, FTP, and CVMFS.
...
To do it, the application was repeated using Solution 2 and 4 on the original machine.
