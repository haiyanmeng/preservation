...
\section{Evaluation}
The section evaluates the connectness of the packaging utility of Solution 4.
To do it, the experiment is repeated using Solution 2 and 4 on the original machine.
Then, two different virtual machines (VM) - one local VM and one amazon EC2 VM, are employed to further verify the reproductivity of Solution 4.

\subsection{Evaluation on the Original Machine}

This section evaluates the correctness of Solution 4 on the original machine and compares the time consumption and data size of Solution 2 and 4 on the original machine. 

To do this, we first repeat the example from scratch using Solution 2 on the the original machine and count the time consumption and data size. 
...
preparation. 
However, under Solution 4, the Software Acquisition stage is not necessray, and all the environment building only takes 3s.
...
obtain filename list and generate the package. 
...
The third column is corresponding to the data size of Solution 4. 
...
In conclusion, Solution 4 can repeat the whole experiment successfully and reduce the total size involved in the experiment to one reasonable size.


\subsection{Evaluation on Different Machines}

To further verify the reproductivity of Solution 4 on other machines, two different virtual machines are employed -
one virtual
machine ~\cite{goldberg1974survey} sharing the same kernel version with the original machine,
and one virtual machine from amazon EC2 ~\cite{amazon2010amazon}.

Table~\ref{table:config-vm} illustrates the configuration of the original machine and each virtual machine. 
