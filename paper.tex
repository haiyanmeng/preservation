...
archive, each sub-item should include the private files and description files of each experiment.

One map file is necessary for the relocation of the data access targets, as
...
De-duplication and sorting techoniques are used to deal with the original filename list, generating one smaller list containing 67,168 items.
...
containing all the necessray file dependnencies, and returns the description
...
system call type. The step also runs under Parrot to access data from
...
However, 
it is impossible to copy the files under certain directories, such as  {\tt /dev}, which can halt the packaging process.
It is meaningless to copy the file under the directory {\tt /proc}, because the process id of one program is random and depends on the
...
\section{Evaluation}
We evaluated the connectness and overhead of the packaging utility of Solution 4.
...
architecture of the curation environment that we imagine for complex
...
instead of ingested. A reearcher that wishes to reproduce a given
